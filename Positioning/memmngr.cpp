...
@module memmngr.cpp - Memory allocation class |

This module contains the memory allocation class for the GISRAP project.
...
Double2DArray new_Double2DArray(Counter rows, Counter cols)
{
//	er << "new " << rows << " " << cols;
...
Double2DArray copy_Double2DArray(Double2DArray &dest, const Double2DArray source, Counter &rows, Counter &cols)
{
	#if !(defined(DOS))
...
Float2DArray new_Float2DArray(Counter rows, Counter cols)
{
...
//	#endif
	...
	for (i=0; i<rows; i++)
   {
	...
	data_array = new float[rows*cols];	// kr 30/06/96 Taken out of if statement for clarity
   										...
   										// enough memory exist.  ??? -- Counter=int=max 65536
	if (data_array == NULL)
   {
      rows = 0;
      cols = 0;
