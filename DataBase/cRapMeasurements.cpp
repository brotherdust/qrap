...
// The constructor
cRapMeasurements::cRapMeasurements (QWidget* parent) : QWidget(parent)
{
...
// Destructor
cRapMeasurements::~cRapMeasurements ()
{
...
void cRapMeasurements::InsertRow ()
{
...
void cRapMeasurements::DeleteRows ()
{
	// Perform the delete
...
void cRapMeasurements::ReloadTable ()
{
...
void cRapMeasurements::ShowAllContents ()
{
	// Make sure that no update signals are triggered
	disconnect(mCurrentTable->GetTable(),SIGNAL(cellChanged(int,int)),0,0);
...
// Excute the search
void cRapMeasurements::ExecuteSearch (string search)
{
	disconnect(mCurrentTable->GetTable(),SIGNAL(cellChanged(int,int)),0,0);
...
void cRapMeasurements::TableSelectionChanged ()
{
...
// Update the database
void cRapMeasurements::UpdateDatabase(int row, int col)
{
	if(!mInsertingRow)
	{
		// Check whether the 
...
void cRapMeasurements::PopulateTableList ()
{
	// Create the items
	QListWidgetItem* MeasDataItem = new QListWidgetItem("Measurement Data Source");
	QListWidgetItem* PosSourceItem = new QListWidgetItem("Position Sources");
	QListWidgetItem* MeasTypeItem = new QListWidgetItem("Measurement Type");
	QListWidgetItem* TestPointsItem = new QListWidgetItem("Test Points");
	QListWidgetItem* MeasurementsItem = new QListWidgetItem("Measurements");
	
	// Setup the items
	MeasDataItem->setData(Qt::UserRole,QVariant("measdatasource"));
	MeasDataItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
	PosSourceItem->setData(Qt::UserRole,QVariant("positionsource"));
	PosSourceItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
	MeasTypeItem->setData(Qt::UserRole,QVariant("meastype"));
	MeasTypeItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
	TestPointsItem->setData(Qt::UserRole,QVariant("testpoint"));
	TestPointsItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
	MeasurementsItem->setData(Qt::UserRole,QVariant("measurement"));
	MeasurementsItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);

	// Add the items to the mTableList
	mTableList->addItem(MeasDataItem);
	mTableList->addItem(PosSourceItem);
	mTableList->addItem(MeasTypeItem);
	mTableList->addItem(TestPointsItem);
	mTableList->addItem(MeasurementsItem);
	
	// Create the different table widgets
	mTables.insert("measdatasource",new cRapTableTab(this,"measdatasource"));
	mTables.insert("positionsource",new cRapTableTab(this,"positionsource"));
	mTables.insert("meastype",new cRapTableTab(this,"meastype"));
	mTables.insert("testpoint",new cRapTableTab(this,"testpoint"));
	mTables.insert("measurement",new cRapTableTab(this,"measurement"));
	...
	mMainLayout->addWidget(mTables["measdatasource"],0,1);
	mTables["measdatasource"]->show();
	mCurrentTable = mTables["measdatasource"];
	...
	mDbCommunicator->PopulateTable(mCurrentTable->GetTable(),"measdatasource","",-1,false);
