...
function(FIND_QCAOSSL_PLUGIN_CPP PLUGIN_REQUIRED)
  ...
  if(QT_INCLUDE_DIR
     AND QT_QTCORE_INCLUDE_DIR
     AND QT_QTCORE_LIBRARY
     AND QCA_INCLUDE_DIR
     AND QCA_LIBRARY
    ...
    set(CODE
      ...
      ")
    set(TESTCPP
        "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/CMakeTmp/qcaossl.cpp")
    file(WRITE ${TESTCPP} "${CODE}")

    set(QCA_INCLUDE_DIRECTORIES
        "-DINCLUDE_DIRECTORIES:STRING=${QT_INCLUDE_DIR};${QT_QTCORE_INCLUDE_DIR};${QCA_INCLUDE_DIR}"
    )
    set(QCA_LINK_LIBRARIES
        "-DLINK_LIBRARIES:STRING=${QT_QTCORE_LIBRARY};${QCA_LIBRARY}")

    try_run(
      RUN_RESULT COMPILE_RESULT ${CMAKE_BINARY_DIR} ${TESTCPP}
...
function(FIND_QCATOOL TOOL_REQUIRED)
  if(NOT QCATOOL_EXECUTABLE)
    ...
    if(MSVC)
      find_program(
        QCATOOL_EXECUTABLE
        NAMES qcatool.exe qcatool2.exe
        PATHS $ENV{LIB_DIR}/bin $ENV{OSGEO4W_ROOT}/bin)
    else()
    ...
    if(NOT "${_qca_plugins}" MATCHES "Available Providers")
      ...
      foreach(_plugin_dir ${_qca_plugins_list})
        ...
        if(EXISTS "${_plugin_dir}"
           AND IS_DIRECTORY "${_plugin_dir}"
           AND NOT QCA_PLUGIN_DIR)
